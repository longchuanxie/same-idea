...
排序执行中两处有据调整：E-001 提前至队首（构建链阻断项，且为用户在途工作收口）；E-008 计划 5 处实改 4 处（DropdownSelect 为双 ref+scroll 捕获的独立变体，强行并入会失真，保留原实现并在 commit 说明）。

| # | 编号 | commit | 验证方式与结果 | 状态 |
|---|---|---|---|---|
| 1 | E-001 | `92687fb` | tsc/eslint 全绿恢复；piperEngine 测试 8→13 例（含 CapacitorHttp 兜底、`-2` 播报、normalize 口径）；`npm run build` 通过；UI 不确定态 chip 依赖真机下载场景，**需人工视觉验证**（步骤见 5.4） | ✅ 关闭（含 1 项需人工复核） |
| 2 | E-002 | `10b812c` | 新增超时测试（fake timers 15s →「AI 服务响应超时」），ai 服务 59 例绿 | ✅ 关闭 |
| 3 | E-003 | `c64bdfd` | store 测试新增 2 例（部分失败含文件名警示/不支持类型计数）+ 既有全失败断言随增强消息更新，26 例绿 | ✅ 关闭 |
| 4 | UX-001 | `6e6351b` | AnnotationList 测试 5→10 例（tab 出现条件/切换/选中/删除/空态）；浏览器真机走查：加书签→面板双 tab→书签卡展示，亮/暗双截图人眼验收通过 | ✅ 关闭 |
| 5 | E-004 | `ee00034` | 新增前置校验测试（不再发请求+明确报错），cloudSync 34 例绿 | ✅ 关闭 |
| 6 | E-005 | `acba3d0` | 新增 `backupSettings.ts` + 2 例单测；恢复端 Partial 合并语义经 `useAppStore.ts:89-92` 核实 | ✅ 关闭 |
| 7 | E-006 | `024e92d` | 新增 fileTransfer.test.ts 3 例（含 10 万字符跨块解码等价性） | ✅ 关闭 |
| 8 | E-007 | `c80275e` | pageRepo 测试 6→7 例（新增前缀兄弟书不误伤边界），fake-indexeddb 实测 | ✅ 关闭 |
| 9 | E-009 | `738bb54` + `680fabf` | 首提后浏览器验收发现**既有缺陷**：`.ink-slider` 原生外观覆盖背景，豁免渐变从未可见 → 追加自绘外观修复；修复前后对比截图验收通过；`lint:tokens` 新 inline hex 规则全仓绿；豁免表/迁移清单回写 | ✅ 关闭（含既有缺陷修复） |
| 10 | E-008 | `8321530` | 新增 useClickOutside 4 例（触发/内点不触发/开关生命周期/闭包稳定），4 消费页测试绿 | ✅ 关闭 |
| 11 | UX-002 | `d4b613a` + `4b3dcf9`（OPDS 标注修正） | 浏览器走查：亮/暗双截图；首验发现 OPDS 被误标「仅测试连接」（实际走 OpdsBrowser 可浏览）→ 当轮修正 | ✅ 关闭 |
| 12 | UX-003 | `90162b5` | 浏览器走查：开面板→切「更多」→关闭→重开，selected tab = 更多（截图） | ✅ 关闭 |
| 13 | UX-004 | `364ccd8` | 浏览器走查：桌面侧栏出现「检索台」，亮/暗双截图 | ✅ 关闭 |
| 14 | E-014 | `a1f9272` | `package.json` JSON 合法性 + tsc | ✅ 关闭 |
| 15 | E-015 | `ad9b1d4` | 全仓 grep 确认死字段 → 删除；tsc 全绿证实无隐性引用 | ✅ 关闭 |
| 16 | E-017 | `9ba6f89` + `4b3dcf9`（类型修复） | 5 例 store 测试绿；回归阶段发现 2 处 tsc 错误（提交时漏跑 tsc）→ 当轮修复，tsc 恢复绿 | ✅ 关闭 |
...
| 门禁 | 基线 | 终态 |
|---|---|---|
| `npm test` | 834/834（106 文件） | **863/863 全绿**（107 文件，净增 29 例） |
| `npm run lint`（含令牌守卫） | 3 错（piperEngine WIP） | **0 错**，并新增 inline hex 守卫规则 |
| `npx tsc --noEmit` | 3 错（同上） | **0 错**（中途 E-017 引入 2 错，回归发现即修，见 4.3 #16） |
| `npm run build` | **失败** | **通过**（7.7s；chunk 体积警告为既有状态） |
| 浏览器冒烟（dev + 真实演示数据） | — | Home/书库侧栏/检索台/设置主题循环/TextReader 书签+面板/设置面板 tab/CustomCloud 帮助逐项过，亮暗双模式截图人眼验收 |

**问题关闭率：16/16 = 100%**（P0 0 项；P1 4 项、P2 9 项、P3 3 项全部关闭；其中 2 项含当轮发现的次生问题并已修复）。

**回归中新发现并即时修复**（按流程编号入账）：① E-009 之既有视觉缺陷（色温渐变从未渲染）；② UX-002 之 OPDS 标注错误；③ E-017 之测试类型错误。三者均未新增编号，随母项提交收口。

### 5.2 实施后动线对比

| 动线 | 实施前 | 实施后 |
|---|---|---|
| b) 选区→批注→**书签**→查看 | 书签列表 0 步不可达（断点） | 顶栏手记按钮 → 书签 tab = **2 步可达**，断点消除（截图验收） |
| c) TTS/阅读设置调整 | 每次重走 3 层且 tab 重置 | 重开直达上次 tab = **3 层→2 层**（截图验收） |
| 自定义云端接入 | 帮助按钮无响应（死角） | 6 协议说明 + 可浏览性标注 + 地址示例（截图验收） |
| 任意页→全库检索（桌面） | 先回首页再点放大镜 = 2 步 | 侧栏直达 = **1 步** |
| 批量导入（部分失败） | 静默丢文件，无任何反馈 | toast 汇总失败文件名；全失败时报错附名单 |
| 音色包兜底下载 | 进度条凭空消失数分钟 | 「语音包下载中」不确定态常驻（逻辑与单测闭环，视觉待真机） |

四原则复查：②死角与折返——书签/帮助两处死角消除，AI 配置往返（UX-005）与同步自动化（UX-006）留遗留；③步数——检索与设置各降 1 步；①④方向感与终点反馈实施前后均良好，未回归。

### 5.3 遗留项清单（本次不实施）

| 项 | 内容 | 建议后续动作 |
|---|---|---|
| UX-005 | AI 未配置引导深链定位 | 设置页支持锚点参数，KnowledgeSection/Ask/Knowledge 跳转带参 |
| UX-006 | 云同步自动触发（退出阅读后防抖） | 先做冲突合并演练与电量评估，加默认关闭开关 |
| E-010 | 线上零日志不可观测 | 立项 debug 日志通道（dev 开/release 关），从分页自校准路径开始接 |
| E-011 | TextReader 3025 行 / useLibraryStore 1125 行 / useVerticalVirtualWindow 917 行拆分 | 专项重构，按 feature-hook 逐个抽离，每步全量回归 |
| E-012 | 分页同步测量阻塞 / React.memo=0 | 分块让步 + memo 试点，必须 Android 真机分页回归 |
| E-013 | 书内检索主线程扫描 | 章文本小写缓存起步，必要时 Worker |
| E-016 | axios/fetch、jszip/libarchive 双栈收敛 | 单独排期，覆盖 OPDS/云同步/解压回归 |
| 测试缺口 | Auth/Reader/KnowledgeHub 页面、15 个 hooks 无测试 | 随对应模块改动顺手补 |
| 需人工验证 | E-001 不确定态 chip（需真实音色包下载场景） | 真机：设置→听书发音→神经网络→确认下载，直连失败切镜像时观察页末「语音包下载中」chip |

### 5.4 回滚指引

- **基线标识**：`comics` @ `b949c04`（实施分支 `optimize-2026-09-17` 自此切出）。
- **整体回滚**：`git checkout comics && git branch -D optimize-2026-09-17`（分支未合入前）。
- **单条回滚**：每项独立 commit（见 4.3 表），可 `git revert <hash>` 单独撤销任一项，互不牵连（E-009 两笔、E-017 两笔需一起 revert）。
- **工作区残留**：`android/app/build.gradle`（用户 WIP，versionCode 19/1.18）保持未提交原样；`.workbuddy/` 为调试产物未入库。
