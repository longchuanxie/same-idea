...
export interface KnowledgeTask {
  ...
  merge(bookTitle: string, partials: { label: string; data: unknown }[], base?: unknown): CharacterGraphData | MindmapNodeData | GlossaryData
  /** 结果为空判断（空结果不落库，直接报「提不出」） */
  isEmpty(data: CharacterGraphData | MindmapNodeData | GlossaryData): boolean
}
  ...
  buildMessages: (input) => [
    { role: 'system', content: CHARACTER_SYSTEM_PROMPT },
    {
      ...
      content: [
        ...
        ...(input.detail === 'core'
          ? ['- 只保留对剧情有实质影响的主要人物（至多 8 人），一次性配角不收']
          : input.detail === 'rich'
            ? ['- 提取出场、被提及且有明确信息的人物（至多 24 人），含重要配角']
            : ['- 只提取本片段中出场、被提及且有明确信息的人物，至多 15 人']),
...
const glossaryTask: KnowledgeTask = {
  ...
  merge: (_bookTitle, partials) =>
    mergeGlossary(partials.map((partial) => partial.data as GlossaryPartial)),
