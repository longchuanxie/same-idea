...
export const ChapterEndPrompt: React.FC<ChapterEndPromptProps> = ({
  ...
  <div className="fixed inset-x-0 bottom-40 z-40 px-margin-mobile pointer-events-none">
    <div className="max-w-max-width-content mx-auto flex items-center justify-between gap-3 rounded-full bg-surface/90 backdrop-blur-md border border-outline-variant/60 shadow-paper-up px-4 py-3 pointer-events-auto">
      <span className="font-label text-label-sm text-on-surface-variant">{title}</span>
      <button
        className="btn-seal px-4 py-2"
        onClick={onNext}
        data-ui-control
      >
        {actionLabel}
