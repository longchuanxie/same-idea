    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const Spacer(),
          Divider(color: morandi.line, height: 1, indent: 16, endIndent: 16),
          _NavItem(
      ...
      child: ListTile(
        ...
        title: Text(
          ...
          style: TextStyle(
            ...
            color: isActive ? morandi.green : morandi.text,
          ),
        ),
        onTap: () {},
      ),
      ...
      child: Row(
        children: [
          ...
          SizedBox(
            ...
            child: TextField(
              decoration: InputDecoration(
                hintText: '搜索项目...',
                hintStyle: TextStyle(fontSize: 13, color: morandi.muted2),
                prefixIcon: Icon(Icons.search, size: 18, color: morandi.muted),
                filled: true,
          ...
          ElevatedButton.icon(
            ...
            label: const Text('新建项目', style: TextStyle(fontSize: 13)),
    ...
    if (state.error != null && state.projects.isEmpty) {
      ...
      return Center(
        child: Column(
          ...
          children: [
            Icon(Icons.error_outline, size: 48, color: morandi.red),
            const SizedBox(height: 16),
            Text(state.error!, style: Theme.of(context).textTheme.bodyLarge),
            const SizedBox(height: 16),
            ElevatedButton(
              onPressed: () => context.read<ProjectListBloc>().add(ProjectListRefreshed()),
    ...
    if (state.projects.isEmpty) {
      ...
      return Center(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(Icons.menu_book_outlined, size: 64, color: morandi.sage),
            const SizedBox(height: 16),
            Text(
              '创建你的第一部小说',
              style: Theme.of(context).textTheme.headlineSmall?.copyWith(color: morandi.muted),
            ),
                ...
                children: [
                  Positioned(
                    ...
                    child: PopupMenuButton<String>(
                      icon: Icon(Icons.more_horiz, color: Colors.white.withOpacity(0.8), size: 20),
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
                      onSelected: (value) {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (project.description.isNotEmpty)
                      ...
                      Text(
                        '暂无简介',
                        style: TextStyle(fontSize: 12, color: morandi.muted2, fontStyle: FontStyle.italic),
                      ),
                    const Spacer(),
                    Row(
                      children: [
                        _StatusChip(
                          label: project.genre.isNotEmpty ? project.genre : '未分类',
                          color: morandi.sand,
                          textColor: morandi.ink,
                        ),
  ...
  void _confirmDelete(BuildContext context) {
    ...
    showDialog<void>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Text('确定删除"${project.title}"吗？此操作不可撤销。'),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            onPressed: () {
              context.read<ProjectListBloc>().add(ProjectListDeleteRequested(projectId: project.id));
...
class _AddProjectTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(10),
        side: BorderSide(color: morandi.line, style: BorderStyle.solid),
      ),
      child: InkWell(
        ...
        child: Center(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Icon(Icons.add_outlined, size: 36, color: morandi.sage),
              const SizedBox(height: 8),
              Text(
                '新建项目',
                style: TextStyle(fontSize: 14, color: morandi.muted),
              ),
            ],
          ),
        ),
